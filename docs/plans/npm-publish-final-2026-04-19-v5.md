# npm Publish — Final Plan (v5)

> **Status:** Drafted 2026-04-19, awaiting Carlo approval
> **Supersedes:** `docs/plans/npm-publish-final-2026-04-19-v4.md`
...
v4 fixed eight correctness issues but still carried seven regressions that surface once you check the actual repo contents and n8n's community-node metadata requirements. v5 applies Carlo's final-before-implementation corrections:
...
7. **Scanner runs pre-publish where possible.** `npx @n8n/scan-community-package n8n-nodes-tokensense` takes a published package name, not a local tarball. If local scanning isn't supported by the scanner version we use, publish `0.1.0-beta.1` first → scan → if clean, publish `0.1.0` from the same commit.
...
curl -sS -X POST https://api.tokensense.io/v1/chat/completions \
  ...
  -d "{
    ...
    \"max_tokens\":5,
    \"metadata\":{\"source\":\"v5-gate\",\"workflow_tag\":\"$MARK\"}
  }"
```

Verify in TokenSense Dashboard → Logs (`workflow_tag = $MARK`) → row exists with `source = v5-gate`.
...
Expected:
```
N8N_VERSION <some 1.x>
N8N_ENGINES_NODE {"node":">=20.15 <=22.x"}      (or similar — record exact range)
...
Use the output of 0.3 to set v5 Sprint 2.7 values:
...
| `engines.node` declared by `n8n@latest` | CI Node version (v5 default) |
|-----------------------------------------|-------------------------------|
...
Same as v4 Sprint 1.3 — unchanged by v5:
  ...
  properties: INodeProperties[] = [
    {
      ...
      required: true,
      typeOptions: {
        // Accept https://host or https://host.path (no /v1 segment at end, no trailing slash required)
        // The test expression also strips a trailing slash so either form works,
        // but we forbid /v1 at the end to keep the test endpoint unambiguous.
        regexp: {
          regex: '^https?://[^/?#]+(?:/(?!v1/?$)[^?#]*)?$',
          errorMessage:
            'Enter bare origin (e.g. https://api.tokensense.io) — do not include /v1 at the end.',
        },
      },
    },
    {
...
Why the regex approach works better than stripping `/v1` in the test expression:
- A single `.replace()` is always safe in the expression engine.
- The regex prevents users from entering `/v1` in the first place (UI-side feedback).
- `normalizeBaseUrl()` still protects runtime calls as a belt-and-braces net.
- If the expression engine rejects even the single `.replace()` for any reason (R8 in v4), fall back to `baseURL: '={{$credentials.endpoint}}'` + trailing-slash regex that also forbids trailing `/` — i.e. the regex enforces the canonical form entirely.
...
Same as v4 Sprint 2.4 — unchanged by v5 (SDK field name, structure, stop-and-replan clause all carry forward).
...
| `engines.node` | `">=18.0.0"` | same | kept as-is; n8n itself requires more, but our package stays permissive |
...
Unchanged from v4: four sub-gates (install, both nodes appear, AI Agent + Chat Model works with metadata in logs, TokenSense AI general node works with metadata in logs). Docker or fresh-npm-install fallback. Record in `docs/audits/sprint-3-smoke-test-2026-04-19.md`.
...
| Rejects local `.tgz` (requires published package) | **Beta-first publish** (see Publish Procedure below): publish `0.1.0-beta.1`, scan, if clean publish `0.1.0` from same commit; if not, fix + repeat |
...
# If clean, promote to 0.1.0 from the same commit (no code changes)
...
- [ ] Sprint 0 (6 gates: 0.1, 0.2, 0.3, 0.5, 0.6) all pass, documented in `docs/audits/sprint-0-feasibility-2026-04-19.md`
